...
public final class RxTest {
    ...
    public void test_buffer() {
        // Setup
        final Predicate<Long> LP = new Predicate<Long>() {
            @Override
        ...
        Flowable<Long> f1 = Flowable.interval(100, TimeUnit.MILLISECONDS)
            .filter(new Predicate<Long>() {
                @Override
                public boolean test(@NotNull Long aLong) throws Exception {
                    return !LP.test(aLong);
                }
            });

        Flowable<Long> f2 = Flowable.interval(500, TimeUnit.MILLISECONDS)
            .filter(new Predicate<Long>() {
                @Override
                public boolean test(@NotNull Long aLong) throws Exception {
                    return LP.test(aLong);
        ...
        Flowable<Long> f3 = Flowable.merge(f1, f2);
        TestSubscriber subscriber = CustomTestSubscriber.create();

        // When
        Flowable.interval(20, TimeUnit.MILLISECONDS)
            .buffer(f3, new Function<Long,Publisher<?>>() {
                @NotNull
                @Override
                public Publisher<?> apply(@NotNull Long aLong) throws Exception {
                    LogUtil.printft("Delay duration: %s", aLong);
                    return Flowable.empty().delay(aLong, TimeUnit.MILLISECONDS);
                }
            })
            .doOnNext(new Consumer<List<Long>>() {
                @Override
                public void accept(@NotNull List<Long> longs) throws Exception {
                    LogUtil.printlnt(longs);
                }
            })
            .subscribe(subscriber);
    ...
    public void test_window() {
        ...
        Flowable.interval(100, TimeUnit.MILLISECONDS)
            .window(10)
            .flatMap(new Function<Flowable<Long>,Publisher<?>>() {
                @NotNull
                @Override
                public Publisher<?> apply(@NotNull Flowable<Long> lf) throws Exception {
                    return lf;
                }
            })
            .doOnNext(new Consumer<Object>() {
